...
class ext_update {
	...
	public function main() {
		...
		foreach ($rows as $row) {
			...
			if (FALSE !== strpos($row['tx_fed_page_controller_action'], 'fluidpages->')) {
				$row['tx_fed_page_controller_action'] = str_replace('fluidpages->', 'fluidpages_intertain->', $row['tx_fed_page_controller_action']);
				$modified = TRUE;
			}
			if (FALSE !== strpos($row['tx_fed_page_controller_action_sub'], 'fluidpages->')) {
				$row['tx_fed_page_controller_action_sub'] = str_replace('fluidpages->', 'fluidpages_intertain->', $row['tx_fed_page_controller_action_sub']);
				$modified = TRUE;
			}
			if (TRUE === $modified) {
		...
		// or inherited a fluidpages_intertain template - adjust FlexForm XML, adding prefix "settings." to all field names
		...
		if (0 < count($count)) {
			...
			$message .= '</ul><br /><br />';
			$message .= 'YOUR PAGES HAVE BEEN ADJUSTED to use 1) the proper extension key "fluidpages_intertain" in all selections which';
			...
			$message .= ' [TOP page] -> fluidpages_intertain -> ... -> OTHERPROVIDER -> ... -> fluidpages_intertain (in other words: if inheritance';
			...
			$message .= ' adjusted (see EXT:fluidpages_intertain/class.ext_update.php for detailed information about which actions to take in this case).';
		}
		return $message;
	}
	...
	protected function adjustFlexFormSourceForAssociatedRecordsInPidRecursive($pid, $count) {
		...
		foreach ($rows as $row) {
			...
			// prevents recursive processing, which means that if you break inheritance and restart using fluidpages_intertain
